...
def cal_macd_boll(dayK):
    ...
    if ((_stock['high'][-3] > _stock['boll_ub'][-3]) | (_stock['high'][-2] > _stock['boll_ub'][-2]) | (
            _stock['high'][-1] > _stock['boll_ub'][-1])):
        cross_above_boll = 1
    if ((_stock['low'][-3] < _stock['boll_lb'][-3]) | (_stock['low'][-2] < _stock['boll_lb'][-2]) | (
            _stock['low'][-1] < _stock['boll_lb'][-1])):
        cross_below_boll = 1
